...
- confirmation that `AGENTS.md`, `CLAUDE.md`,
  ...
  content, not a literal text match. `CLAUDE.md` is a full effective-content
  mirror and does not import `@AGENTS.md`. Cursor's effective content is the
  union of `.cursor/rules/*.mdc` (Cursor complements only) and Cursor's
  native root `AGENTS.md` auto-apply (no `@AGENTS.md` inside `.mdc`).
