...
| **採用アダプター** | テンプレートを **採用したプロジェクト**側で、同期・運用・フィードバックを担う人またはチーム。Clean Architecture の Adapter ではない。 |
| **Adapter（Clean Architecture）** | ポートを実装する外側レイヤのコンポーネント（DB、SDK、ファイル I/O など）。「採用アダプター」と区別する。 |
...
過去に「アダプター（実行者）」「アダプター（プロジェクト実行者）」と書いていた箇所は、
すべて **採用アダプター** に読み替える。
