...
第三の道は、決定点を Adjudicator が所有し、生成と機械検証をエージェント（と決定的ツール）に委ねる構造的協働である。役割の詳細は `docs/collaboration/ai-human-scheme.md` と [sandbox](2026-07-07-rationale-saas-agent-as-sandbox.md) に譲り、本稿は **決定点・フェーズ・責任**に絞る。境界が文書化されていること自体が前提である（ADR 0003）。
