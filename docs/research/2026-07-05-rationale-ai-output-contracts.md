...
ソフトウェアエンジニアリングにおいて「契約（Contract）」という概念は、単なるインターフェースの定義を超えた深い哲学を持っている。Bertrand Meyer が提唱した「契約による設計（Design by Contract）」は、呼び出し元と呼び出し先の間に事前条件、事後条件、不変条件を厳密に定義することで、ソフトウェアの信頼性を担保しようとした。この哲学は、AI時代において新たな次元の重要性を帯びている。
...
## 「ユーザースペースを壊すな」：インターフェースの絶対性

契約による設計を実践する上で最も強力な歴史的教訓の一つは、Linuxカーネルにおける Linus Torvalds の絶対的なルール「ユーザースペースを壊すな（WE DO NOT BREAK USERSPACE!）」に見出すことができる。カーネル（提供側）の実装をどれほど美しくリファクタリングしようとも、その結果としてユーザースペースの既存のプログラム（利用側）が動かなくなるような変更は、アーキテクチャへの重大な裏切りとして即座に却下される。

AI協働において、この「インターフェースの絶対性」の概念は極めて重要である。AIエージェントはコードベース全体を俯瞰し、高速にコードを書き換える能力を持つ。そのため、「内部実装を綺麗にするついでに、関数の引数や戻り値の型（契約）も書き換えてしまおう」という誘惑に弱く、それを無邪気に実行してしまう。しかし、そのインターフェースに依存している他のコンポーネント（ユーザースペースに相当）への影響を正確に予見できなければ、システム全体が破壊される。

AIにコードを生成・リファクタリングさせる際、私たちは常に「カーネル開発者」に対するのと同じ厳格さを要求しなければならない。「内部の実装（Adapterなど）はどう最適化しても良いが、利用側との契約（Interface）や、外部からの呼び出し規約は勝手に変更してはならない」。契約とは、単なるJSONスキーマの型チェックにとどまらない。それは提供側と利用側の間の強固な信頼の境界線であり、AIの生成パワーを安全に封じ込めるための絶対的な防壁なのである。
...
`docs/architecture/external-resource-adoption-contract.md` が外部リソースの `intake -> checked -> accepted | rejected | needs_recheck -> adopted` というライフサイクルを置くのも、同じ発想である。由来が AI か人間かにかかわらず、外部由来のものをプロジェクトの信頼境界へ入れるには、厳密な採用の記録が要る。
...
2. **ソフトウェア工学・哲学**
   ...
   - Torvalds, L. "Re: [Regression w/ patch] Media commit causes user space to misbehave." LKML, 2012-12-23. https://lkml.org/lkml/2012/12/23/75 （未検証：アーカイブがボットアクセスを遮断するため本文未確認。"WE DO NOT BREAK USERSPACE" の原典として広く参照されるメール）
