...
AIにコードを生成・リファクタリングさせる際、私たちは常に「カーネル開発者」に対するのと同じ厳格さを要求しなければならない。「内部の実装（Adapterなど）はどう最適化しても良いが、利用側との契約（Interface）や、外部からの呼び出し規約は勝手に変更してはならない」。契約とは、単なるJSONスキーマの型チェックにとどまらない。それは提供側と利用側の間の強固な信頼の境界線であり、AIの生成パワーを安全に封じ込めるための防壁なのである。
