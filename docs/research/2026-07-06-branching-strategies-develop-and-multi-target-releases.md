...
この根本的な問いは、ある開発セッション（LISS-0005）での実践的な失敗から提起された。第2アダプターによる開発セッションにおいて、エージェントは `develop` ブランチを無邪気に作成し、メインストリームへのマージを先送りした。結果として feature ブランチに未統合の変更スタックが溜まり続けたのだ。これを受けて Adjudicator（人間の裁定者）は問いかけた。「テンプレートは `develop` という存在を無条件に禁止すべきか？」
