...
`docs/research/` の編集は、`llm-project-template` が提供するプロジェクトの運用テンプレート（運用ルールそのもの）に変更を加えるものではない。references をいくら増やしても、essay をどれほど厚く哲学的に書き直しても、それだけでは一切の規範変更にはならない。規範の変更が必要なら、別途 ADR や collaboration 文書の差分として提出し、Referee のレビューを通過しなければならない。この峻別された線引きがあるからこそ、人間は安心して、システムの挙動を壊す恐怖なしに、思想や背景を深く豊かに書くことができる。
