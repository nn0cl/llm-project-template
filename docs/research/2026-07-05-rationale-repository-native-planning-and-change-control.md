...
したがって本テンプレートでは、規範側で ADR を **従うべき拘束的な決定**として扱い、契約ファイル（`AGENTS.md` 等）がその執行を現場に下ろし、Issue / Trace が改訂手続きを担う（ADR 0005 / 0006）。これは物理的な強制ではない（[エージェントはサンドボックスである](2026-07-07-rationale-saas-agent-as-sandbox.md)）。Lessig の "Code is Law" は、コードが行動を形作るという有名な類推であり、ここで「リポジトリ規約が生成コードを方向づける」と読む助けにはなるが、**法制度そのものではない**。レビューと CI が支えるのは、比喩上の「法体系」ではなく、文書化された決定と手続きである。
